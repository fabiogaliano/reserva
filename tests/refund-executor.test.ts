...
describe('attemptRefund with an execution claim (the scheduled-reconciler branch)', () => {
  ...
  it('a retryable Stripe failure resolves failed with a backoff next_attempt_at (attempt 1)', async () => {
    const seeded = booking({ id: 'refund-exec-retryable', status: 'cancelled', stripePaymentIntent: 'pi_exec_retryable' });
    ...
    await repo.claimRefundOperation({ id: 'op-1', bookingId: seeded.id, paymentIntent: seeded.stripePaymentIntent, choice: 'full', requestedAt: '2026-08-14T09:00:00.000Z' });

    const outcome = await attemptRefund(context, seeded, 'op-1', 'full', seeded.stripePaymentIntent, { attemptNumber: 1 });
  ...
  it('a retryable Stripe failure abandons once attemptNumber reaches the cap, with no next_attempt_at', async () => {
    const seeded = booking({ id: 'refund-exec-exhausted', status: 'cancelled', stripePaymentIntent: 'pi_exec_exhausted' });
    const { repo, context } = contextFor([seeded], {
      payments: {
        createCheckout: async () => ({ url: '', sessionId: '' }),
        parseWebhook: async () => { throw new Error('unused'); },
    ...
    await repo.claimRefundOperation({ id: 'op-2', bookingId: seeded.id, paymentIntent: seeded.stripePaymentIntent, choice: 'full', requestedAt: '2026-08-14T09:00:00.000Z' });

    const outcome = await attemptRefund(context, seeded, 'op-2', 'full', seeded.stripePaymentIntent, { attemptNumber: SIDE_EFFECT_MAX_ATTEMPTS });
  ...
  it('a permanent (non-retryable) Stripe failure abandons immediately, even on the first attempt', async () => {
    const seeded = booking({ id: 'refund-exec-permanent', status: 'cancelled', stripePaymentIntent: 'pi_exec_permanent' });
    ...
    await repo.claimRefundOperation({ id: 'op-3', bookingId: seeded.id, paymentIntent: seeded.stripePaymentIntent, choice: 'full', requestedAt: '2026-08-14T09:00:00.000Z' });

    const outcome = await attemptRefund(context, seeded, 'op-3', 'full', seeded.stripePaymentIntent, { attemptNumber: 1 });
    expect(outcome).toMatchObject({ kind: 'failed', retryable: false });
  ...
  it('a successful claimed attempt resolves succeeded exactly like the unclaimed HTTP path', async () => {
    const seeded = booking({ id: 'refund-exec-success', status: 'cancelled', stripePaymentIntent: 'pi_exec_success' });
    const { repo, context } = contextFor([seeded], {
      payments: {
        createCheckout: async () => ({ url: '', sessionId: '' }),
        parseWebhook: async () => { throw new Error('unused'); },
        getSession: async () => ({ status: 'open' }),
        refund: async () => ({ refundId: 're_exec_success', amountCents: seeded.priceCents }),
      },
    });
    await repo.claimRefundOperation({ id: 'op-4', bookingId: seeded.id, paymentIntent: seeded.stripePaymentIntent, choice: 'full', requestedAt: '2026-08-14T09:00:00.000Z' });

    const outcome = await attemptRefund(context, seeded, 'op-4', 'full', seeded.stripePaymentIntent, { attemptNumber: 1 });
  ...
  it('a claimed attempt still skips Stripe entirely for choice none and for a missing payment intent', async () => {
    const noneBooking = booking({ id: 'refund-exec-none', status: 'cancelled', stripePaymentIntent: null });
    ...
    const missingIntentBooking = booking({ id: 'refund-exec-missing-intent', status: 'cancelled', stripePaymentIntent: null });
    ...
    expect(missingOutcome).toEqual({ kind: 'payment_intent_missing' });
