...
// The runtime factories, provider ports, and admin-auth helpers live only at
// '@reservajs/astro/runtime': they belong to the request-time module, and a build-time import of
// this barrel must not pull them (or their Cloudflare types) along.
//
// The UI copy seam likewise lives at its own '@reservajs/astro/ui' subpath, not here: a page or
// component that only needs message helpers must not pull this barrel's build-time integration
// (and Astro's config machinery with it) into a browser/Worker bundle.
//
