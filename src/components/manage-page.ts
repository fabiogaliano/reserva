...
export function renderManagePage(payload: Record<string, unknown>): string {
  ...
  const cancelForm = canCancel ? `<form method="post" action="/booking/manage"><input type="hidden" name="action" value="cancel"><input type="hidden" name="${cancelTokenField}" value="${escape(token)}">${refundControl}<button type="submit">Cancel booking</button></form>` : '';
  const rescheduleForm = canReschedule ? `<form method="post" action="/booking/manage"><input type="hidden" name="action" value="reschedule"><input type="hidden" name="${cancelTokenField}" value="${escape(token)}"><label>New start <input name="newStart" type="datetime-local" required></label><button type="submit">Reschedule</button></form>` : '';
  const noShowForm = canNoShow ? `<form method="post" action="/booking/manage"><input type="hidden" name="action" value="no-show"><input type="hidden" name="operatorToken" value="${escape(token)}"><button type="submit">Mark no-show</button></form>` : '';
