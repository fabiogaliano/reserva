...
repo. These entries record two places where a spec review found the spec itself has a
gap that the implementation cannot resolve alone. Each states the current behavior and
a recommendation for the next spec revision, so it doesn't get re-litigated.
...
**Decision / recommendation.** Work package 01 (`docs/tmp/bookit-fixes/01-code-fixes.md`,
Fix 3) changes both customer routes to check status first and return
...
`handleOperatorNoShow`) already use. This is recorded as the adopted/intended
refinement: an additive contract clarification, not a spec violation, that the next
spec revision should adopt by documenting `409 invalid_transition` alongside
`past_cutoff` for these two routes.
