#!/usr/bin/env bun
// Proves the BUILT smoke-site output serves traffic through the pinned @astrojs/cloudflare
// adapter's `astro preview` entrypoint. The Playwright suite only ever runs `astro dev`, so a bug
// that only manifests in the built worker (a route missing from the compiled manifest, an asset
// never emitted) would pass every existing suite unnoticed.
...
// Fresh D1 state every run: a stale prior run's schema/rows must never leak into this run's
// readiness/availability assertions.
...
const previewEnv: NodeJS.ProcessEnv = {
  ...
  // Astro's CLI auto-backgrounds `preview` when it thinks an agent is driving it; any truthy value
  // here defeats that heuristic, so this stays the plain foreground process this script spawns,
  // probes, and terminates itself.
...
async function waitForReady(timeoutMs: number): Promise<void> {
  ...
  for (;;) {
    ...
    try {
      ...
      // behind it, is actually up — hitting a real route, not just an accepting socket.
